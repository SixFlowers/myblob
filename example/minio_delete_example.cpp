...
int main() {
    ...
    std::cout << "删除 MinIO 对象: http://192.168.17.1:9000/test1/" << objectName << std::endl;

    cloud::Transaction transaction(provider.get());
    uint8_t buffer[8192] = {};

    if (!transaction.deleteObjectRequest(objectName, buffer, sizeof(buffer))) {
        std::cerr << "创建 MinIO DELETE 请求失败" << std::endl;
        return 1;
    }
