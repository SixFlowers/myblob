...
namespace myblob
{
  ...
  namespace cloud
  {
    ...
    public:
      ...
      { // AWS凭证
        // IAM用户名
        std::string iamUser; // 不是按用户区分的，是按 EC2 实例区分的
        // Access keyID
        std::string keyId;
        // Secret Access Key
        std::string secret;
    ...
    protected:
      Settings _settings;
      // 全局凭证，从EC2元数据服务获取的IAM凭证，包含Access Key ID、Secret Access Key和Session Token等信息，用于所有普通的S3请求（GET/PUT/DELETE等）。每个AWS实例都有一份独立的全局凭证。
      std::shared_ptr<Secret> _globalSecret; // 普通凭证(iam获取的json信息)，用于所有普通的 S3 请求（GET/PUT/DELETE 等）;是每个AWS实例各有一份
      // 全局会话凭证
      std::shared_ptr<Secret> _globalSessionSecret; // S3 Express 会话凭证，一般用不到是 AWS 2023 年推出的一种高性能 S3 存储层级，
      // 比标准 S3 延迟更低、吞吐更高，专门为 AI/ML、数据分析等场景设计。它需要额外的认证步骤（getSessionToken），返回的凭证就叫"会话凭证"。
      // 多部分上传分片大小
      uint64_t _multipartUploadSize = 128ull << 20; // 128MB
      // 凭证互斥锁
      std::mutex _mutex;
      // 线程本地凭证
      thread_local static std::shared_ptr<Secret> _secret; // 普通凭证(iam获取的json信息)，用于所有普通的 S3 请求（GET/PUT/DELETE 等）
      // 线程本会话凭证
      thread_local static std::shared_ptr<Secret> _sessionSecret; // S3 Express 会话凭证，一般用不到
      // 有效实例指针 
      thread_local static AWS *_validInstance; // 可能存在多个AWS实例
    public:
      explicit AWS(const RemoteInfo &info,
                   network::ConnectionManager &conn_mgr,
                   network::HttpClient &http_client)
          : Provider(info.endpoint, info.port, conn_mgr, http_client, info.provider), _settings({info.bucket, info.region, info.endpoint, info.port, info.https, info.zonal}), _mutex()
      {
        assert(info.provider == CloudService::AWS || info.provider == CloudService::MinIO || info.provider == CloudService::Oracle ||
               info.provider == CloudService::IBM); // 检查是否支持AWS
        // 指定存储桶
        assert(!info.bucket.empty());
        assert(info.provider == CloudService::AWS || (!info.endpoint.empty() ||
                                                      !info.region.empty())); // 检查兼容aws的服务是否有足够的信息工作(必须有地址和地区)
        type_ = info.provider;
      }
      ...
      [[nodiscard]] bool isHTTPS() const override { return _settings.endpoint.empty() ? true : _settings.https; }
      // 初始化缓存
    ...
    private:
      ...
      /// 下载实例元数据
      [[nodiscard]] static std::unique_ptr<utils::DataVector<uint8_t>> downloadInstanceInfo(const std::string &info = "instance-type");//默认参数是获取实例类型
      /// 获取会话令牌
